...
class AbstractFasterRCNNModel (FasterRCNNBaseModel):
    ...
    def detect_forward(self, images, image_windows, override_class=None, return_rpn_rois=False):
        ...
        :param override_class: int or None; override class ID to always be this class
        :param return_rpn_rois: if True, return the ROIs generated by the RPN as well

        :return: (detections, n_dets_per_sample) if not return_rpn_rois else:
                 (detections, n_dets_per_sample, rpn_detections, n_rois_per_sample) where
            detections: `RCNNDetections` named tuple that has the following attributes:
                boxes: [batch, num_detections, 4] detection boxes
                class_ids: [batch, num_detections] detection class IDs
                scores: [batch, n_rois_after_nms] detection confidence scores
            n_dets_per_sample: [batch] number of detections per sample in the batch

            rpn_detections: `RPNDetections` named tuple that has the following attributes:
                boxes: [batch, n_rois_after_nms, 4] RPN ROIs
                scores: [batch, n_rois_after_nms] RPN scores
            n_rois_per_sample: [batch] number of RPN ROIs per sample in the batch
        """
        image_size = images.shape[2:]

        # rpn_feature_maps: [batch, channels, height, width]
        # mrcnn_feature_maps: [batch, channels, height, width]
        # rpn_bbox_deltas: [batch, anchors, 4]
        # rpn_rois: [batch, n_rois_after_nms, 4] (normalized co-ordinates)
        # roi_scores: [batch, n_rois_after_nms]
        # n_rois_per_sample: [batch]
        rpn_feature_maps, rcnn_feature_maps, rpn_bbox_deltas, rpn_rois_nrm, roi_scores, \
            n_rois_per_sample = self.rpn_detect_forward(images)

        # det_boxes: [batch, num_detections, (y1, x1, y2, x2)] in image coordinates
        # det_class_ids: [batch, num_detections]
        # det_scores: [batch, num_detections]
        det_boxes, det_class_ids, det_scores, n_dets_per_sample = self.rcnn_detect_forward(
            image_size, image_windows, rcnn_feature_maps, rpn_rois_nrm, n_rois_per_sample, override_class=override_class)

        if return_rpn_rois:
            height, width = image_size
            scale = torch.tensor([height, width, height, width], dtype=torch.float, device=rpn_rois_nrm.device)

            return (RCNNDetections(boxes=det_boxes, class_ids=det_class_ids, scores=det_scores), n_dets_per_sample,
                    RPNDetections(boxes=rpn_rois_nrm * scale[None, :], scores=roi_scores), n_rois_per_sample)
        else:
            return RCNNDetections(boxes=det_boxes, class_ids=det_class_ids, scores=det_scores), n_dets_per_sample


    def detect_forward_np(self, images, image_windows, override_class=None, return_rpn_rois=False):
        ...
        :return: [detections...] if return_rpn_rois is False
            or ([detections...], [rpn_out...]) if return_rpn_rois is True

            where [detections...] is a list of detections, one per sample, where each detection is a
                `RCNNDetections` named tuple that has the following attributes:
                    boxes: [1, detections, [y1, x1, y2, x2]] detection boxes
                    class_ids: [1, detections] detection class IDs
                    scores: [1, detections] detection confidence scores
            where [rpn_out...] is a list of RPN outputs, one per sample, where each RPN output is a
                `RPNDetections` named tuple that has the following attributes:
                    boxes: [1, rpn_rois, [y1, x1, y2, x2]] RPN detection boxes
                    scores: [1, rpn_rois] RPN objectness scores
        """
        if return_rpn_rois:
            t_rcnn_dets, n_dets_per_sample, t_rpn_dets, n_rois_per_sample = \
                self.detect_forward(images, image_windows, override_class=override_class, return_rpn_rois=return_rpn_rois)
        else:
        ...
        det_class_ids_np = t_rcnn_dets.class_ids.cpu().numpy()
        det_scores_np = t_rcnn_dets.scores.cpu().numpy()

        if return_rpn_rois:
            rpn_rois_np = t_rpn_dets.boxes.cpu().numpy()
            rpn_scores_np = t_rpn_dets.scores.cpu().numpy()

            rcnn_dets = split_detections(n_dets_per_sample, det_boxes_np, det_class_ids_np, det_scores_np)
            rpn_dets = split_detections(n_rois_per_sample, rpn_rois_np, rpn_scores_np)

            return ([RCNNDetections(boxes=d[0], class_ids=d[1], scores=d[2]) for d in rcnn_dets],
                    [RPNDetections(boxes=d[0], scores=d[1]) for d in rpn_dets])
        else:
            rcnn_dets = split_detections(n_dets_per_sample, det_boxes_np, det_class_ids_np, det_scores_np)
            return [RCNNDetections(boxes=d[0], class_ids=d[1], scores=d[2]) for d in rcnn_dets]
