...
class AbstractFasterRCNNModel (FasterRCNNBaseModel):
    ...
    def detect_forward(self, images, image_windows, override_class=None, return_rpn_rois=False):
        ...
        :return: (det_boxes, det_class_ids, det_scores, n_dets_per_sample) if not return_rpn_rois else:
                 (det_boxes, det_class_ids, det_scores, n_dets_per_sample, rpn_rois, rpn_scores, n_rois_per_sample) where
            det_boxes: [batch, num_detections, 4] detection boxes
            det_class_ids: [batch, num_detections] detection class IDs
            ...
            rpn_rois: [batch, n_rois_after_nms, 4] RPN boxes
            rpn_scores: [batch, n_rois_after_nms] RPN scores
        ...
        if return_rpn_rois:
            ...
            return (det_boxes, det_class_ids, det_scores, n_dets_per_sample,
                    rpn_rois_nrm * scale[None, :], roi_scores, n_rois_per_sample)
        else:
            return det_boxes, det_class_ids, det_scores, n_dets_per_sample
    ...
    def detect_forward_np(self, images, image_windows, override_class=None, return_rpn_rois=False):
        ...
        :return: [detections...] if return_rpn_rois is False
            ...
            where [detections...] is a list of detections, one per sample, where each detection is a tuple of:
            ...
            where [rpn_out...] is a list of RPN outputs, one per sample, where each RPN output is a tuple of:
                rpn_boxes: [1, rpn_rois, [y1, x1, y2, x2]] RPN detection boxes
                rpn_scores: [1, rpn_rois] RPN objectness scores
        """
        if return_rpn_rois:
            det_boxes, det_class_ids, det_scores, n_dets_per_sample, rpn_rois, rpn_scores, n_rois_per_sample = \
        ...
        if return_rpn_rois:
            ...
            return (split_detections(n_dets_per_sample, det_boxes_np, det_class_ids_np, det_scores_np),
                    split_detections(n_rois_per_sample, rpn_rois_np, rpn_scores_np))
        else:
            return split_detections(n_dets_per_sample, det_boxes_np, det_class_ids_np, det_scores_np)
