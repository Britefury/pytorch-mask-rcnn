...
#define CUDA_1D_KERNEL_LOOP(i, n)                              \
    ...
    __device__ float bilinear_interpolate(const float* image_in, const int height, const int width,
            ...
            float v1 = image_in[y_low * width + x_low];
            float v2 = image_in[y_low * width + x_high];
            float v3 = image_in[y_high * width + x_low];
            float v4 = image_in[y_high * width + x_high];
    ...
    __global__ void ROIAlignForward(const int nthreads, const float* image_in, const int height, const int width,
                                    const int channels, const int aligned_height, const int aligned_width, const int sampling_ratio,
                                    const int *sample_indices, const float* boxes, float* crops_out) {
        CUDA_1D_KERNEL_LOOP(index, nthreads) {
            ...
            const float* offset_boxes = boxes + n * 4;
            int roi_batch_ind = sample_indices[n];

            // Do not using rounding; this implementation detail is critical
            float roi_start_h = offset_boxes[0] * height;
            float roi_start_w = offset_boxes[1] * width;
            float roi_end_h = offset_boxes[2] * height;
            float roi_end_w = offset_boxes[3] * width;
            ...
            const float* offset_image_in =
                image_in + (roi_batch_ind * channels + c) * height * width;
            ...
            for (int iy = 0; iy < roi_bin_grid_h; iy++) // e.g., iy = 0, 1
            {
                ...
                for (int ix = 0; ix < roi_bin_grid_w; ix++) {
                    ...
                    float val = bilinear_interpolate(
                        offset_image_in, height, width, y, x, index);
                    output_val += val;
                }
            }
            output_val /= count;

            crops_out[index] = output_val;
        }
    }
    ...
    __global__ void ROIAlignBackward(const int nthreads, const float* image_out_diff, const int height, const int width,
                                     const int channels, const int aligned_height, const int aligned_width, const int sampling_ratio,
        ...
        CUDA_1D_KERNEL_LOOP(index, nthreads) {
            ...
            float* offset_bottom_diff =
                image_in_diff + (roi_batch_ind * channels + c) * height * width;

            int top_offset = (n * channels + c) * aligned_height * aligned_width;
            const float* offset_top_diff = image_out_diff + top_offset;
